...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	public ClusterClientProvider<ApplicationId> deployJobCluster(
		...
		try {
			 ...
			 */
			return deployInternal(
				clusterSpecification,  // 集群的资源情况
				"Flink per-job cluster", // 默认的应用名称
				getYarnJobClusterEntrypoint(), // Application Master启动类:  YarnJobClusterEntrypoint
				jobGraph,  // JobGraph
				detached   // 是否 detach 模式
			);
	...
	private ClusterClientProvider<ApplicationId> deployInternal(
		...
		/**
		 * 根据当前yarn空闲的资源和per-job申请的资源，返回一个可以使用的有效资源（validClusterSpecification）
		 */
		...
		/**
		 *  非常关键的一步， 启动 ApplicationMaster
		 *  也就是在yarn分配某一个container上，申请运行 YarnJobClusterEntrypoint
		 *  *************************************************************************************
		 *  直接转去看  YarnJobClusterEntrypoint.main()  就行了， startAppMaster()都是一些yarn相关的琐碎代码
		 *  *************************************************************************************
		 */
		...
		/**
		 * 启动完appMaster之后，记录下appMaster（也就是job master）的ip和port
		 */
		setClusterEntrypointInfoToConfig(report);


		/**
		 * 返回一个  RestClusterClient 的对象
		 * 里面封装了 submitJob 一些提交 jobGraph的方法
		 * 像yarn-session的部署模式，后面可以用client不停的提交job
		 */
		return () -> {
			try {
				return new RestClusterClient<>(flinkConfiguration, report.getApplicationId());
