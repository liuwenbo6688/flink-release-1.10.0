...
public class StreamGraph implements Pipeline {
	...
	/**
	 * StreamGraph里所有 <ID,StreamNode> 映射
	 */
	private Map<Integer, StreamNode> streamNodes;

	/**
	 * 保存所有 source operator的ID
	 */
	private Set<Integer> sources;
	/**
	 * 保留所有 sink operator的ID
	 */
	private Set<Integer> sinks;

	/**
	 * 虚拟节点， Select、SideOutput、keyby 操作都是生成虚拟节点，不包含具体的operator
	 */
	private Map<Integer, Tuple2<Integer, List<String>>> virtualSelectNodes;
	private Map<Integer, Tuple2<Integer, OutputTag>> virtualSideOutputNodes;
	private Map<Integer/*虚拟节点ID*/, Tuple3<Integer /*虚拟节点的上游ID*/ , StreamPartitioner<?>, ShuffleMode>> virtualPartitionNodes;
	...
	public <IN, OUT> void addOperator(
		 ...
		 * 不管是SourceStreamTask还是OneInputStreamTask 都是最终执行的Task的invoke()方法
		 */
	...
	private void addEdgeInternal(Integer upStreamVertexID,
		...
		/**
		 * 前三个if判断，处理上游节点是 虚拟节点的情况
		 * Select、SideOutput、keyby 操作都是生成虚拟节点，不包含具体的operator
		 */
		...
		/**
		 * 这里才是
		 * 真正的添加StreamEdge，连接上下游的 StreamNode
		 */
		else {
