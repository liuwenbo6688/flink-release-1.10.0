	...
	private boolean performCheckpoint(
		...
		if (isRunning) {
			actionExecutor.runThrowing(() -> {
				...
				/**
				 * 1.  执行operatorChain的准备checkpoint屏障的逻辑
				 */
				operatorChain.prepareSnapshotPreBarrier(checkpointId);

				// Step (2): Send the checkpoint barrier downstream
				/**
				 * 2. 遍历所有的输出，将checkpoint屏障发送给下游
				 */
		...
		public void run() {
			...
			try {
				...
				for (Map.Entry<OperatorID, OperatorSnapshotFutures> entry : operatorSnapshotsInProgress.entrySet()) {
					...
					// 创建finalizedSnapshots，创建的时候执行快照过程中的异步方法
				...
				if (asyncCheckpointState.compareAndSet(CheckpointingOperation.AsyncCheckpointState.RUNNING,
					CheckpointingOperation.AsyncCheckpointState.COMPLETED)) { // 将快照状态从运行中设置为已完成

					/**
					 *  汇报快照过程已经完成
					 *
					 *  也就是 Snapshot确认消息发送的起始位置
					 */
	...
	private static final class CheckpointingOperation {
		...
		public void executeCheckpointing() throws Exception {
			...
			try {
				 ...
				 * 就是调用AbstractStreamOperator的snapshotState()方法, 其实最终会调用的 userFunction的snapshotState()方法
				...
				/**
				 * 异步封装了完成执行snapshot过程的逻辑
				 * execute 执行线程
				 * 看AsyncCheckpointRunnable的run方法
				 */
