...
public class Kafka09Fetcher<T> extends AbstractFetcher<T, TopicPartition> {
	...
	/**
	 * Handover 负责在 KafkaConsumerThread 和 KafkaFetcher 之间传递数据
	 */
	...
	public Kafka09Fetcher(
		...
		this.consumerThread = new KafkaConsumerThread(
				LOG,
				handover, // 把handover传递到kafka消费线程中，作为数据中转手
				kafkaProperties,
	...
	public void runFetchLoop() throws Exception {
		try {
			...
			// 启动 kafka 消费线程，定期从kafkaConsumer拉取数据并转交给handover对象
			...
			while (running) {
				...
				for (KafkaTopicPartitionState<TopicPartition> partition : subscribedPartitionStates()) {
					...
					/**
					 *  调用 kafka的api， 获取属于该分区的数据records
					 */
					...
					for (ConsumerRecord<byte[], byte[]> record : partitionRecords) {
						...
						// 反序列化数据为bean, 此deserializer需要实现 KafkaDeserializationSchema
