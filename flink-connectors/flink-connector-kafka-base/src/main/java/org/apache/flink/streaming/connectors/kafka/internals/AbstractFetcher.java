...
public abstract class AbstractFetcher<T, KPH> {
		...
		if (record != null) {
			...
			if (timestampWatermarkMode == NO_TIMESTAMPS_WATERMARKS) {
				...
				synchronized (checkpointLock) {
					...
					partitionState.setOffset(offset);// 更新分区状态的state
				}
			}
			else if (timestampWatermarkMode == PERIODIC_WATERMARKS) {
				// 如果是周期发送watermark,  重点看一下 PERIODIC
				emitRecordWithTimestampAndPeriodicWatermark(record, partitionState, offset, timestamp);
			}
			else {
				// 反之是 PunctuatedWatermark
	...
	private void emitRecordWithTimestampAndPeriodicWatermark(
		...
		synchronized (withWatermarksState) {
			/**
			 * 获取timestamp
			 * 因为 TimestampsAndPeriodicWatermarksOperator 具有定时触发发送watermark的任务，需要考虑并发访问的问题
	...
	private void emitRecordWithTimestampAndPunctuatedWatermark(
		...
		// punctuated extractor 不用考虑并发访问的问题
		...
		if (newWatermark != null) {
			// 此处发送watermark
			// watermark 值为此subtask负责的各个分区的watermark的最小值
