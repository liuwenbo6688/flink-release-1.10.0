...
public class JobManagerRunnerImpl implements LeaderContender, OnCompletionActions, JobManagerRunner {
	...
	public JobManagerRunnerImpl(
		...
		try {
			...
			/**
			 * 使用JobMaster的工厂创建 JobMaster，JobMaster就是真正的JobManager
			 */
	...
	public void start() throws Exception {
		try {
			/**
			 * 启动选举服务，
			 * 当成为leader时，执行grantLeadership()方法
			 *
			 */
	...
	public void grantLeadership(final UUID leaderSessionID) {
		synchronized (lock) {
			 ...
			 * 		相当于flatMap，用来连接两个CompletableFuture
			 */
		...
		return jobSchedulingStatusFuture.thenCompose(
			jobSchedulingStatus -> {
				if (jobSchedulingStatus == JobSchedulingStatus.DONE) {
					...
					/**
					 *  当前job的调度状态不为DONE，说明没有完成，继续执行启动 JobMaster的方法
					 */
