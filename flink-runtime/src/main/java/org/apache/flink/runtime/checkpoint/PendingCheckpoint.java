...
public class PendingCheckpoint {
	...
	public boolean areTasksFullyAcknowledged() {
		// 任务没有被废弃，并且notYetAcknowledgedTasks为空（所有的task都已被确认）
	...
	public TaskAcknowledgeResult acknowledgeTask(
		...
		synchronized (lock) {
			...
			/**
			 * 非常重要的一步
			 * 从notYetAcknowledgedTasks 集合中移除已确认的task
			 * notYetAcknowledgedTasks 保存了所有未确认的task
			 * 之后判断这个PendingCheckpoint是否完成的依据就是 notYetAcknowledgedTasks为空
			 */
