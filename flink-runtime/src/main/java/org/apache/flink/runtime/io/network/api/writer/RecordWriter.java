 ...
 *
 * RecordWriter 负责把数据序列化，然后写入到缓存中
 */
public abstract class RecordWriter<T extends IOReadableWritable> implements AvailabilityProvider {
	...
	/**
	 * 数据发送端最重要的角色是 RecordWriter 和 ResultPartition
	 * RecordWriter负责将数据流中的元素序列化，存入到 ResultSubpartition 中
	 *
	 * 一个数据处理流程(Task)对应着一个ResultPartition
	 * ResultPartition 的数据需要发送数据到多个下游Channel(对应着下游多个Task)
	 * ResultPartition中的数据专门为下游不同的接收者做了分组，这个分组叫做 ResultSubpartition
	 *
	 */
	...
	protected boolean copyFromSerializerToTargetChannel(int targetChannel) throws IOException, InterruptedException {
		...
		// 此处 Serializer 为 SpanningRecordSerializer
		// reset()方法将serializer内部的databuffer的position重置为0
		serializer.reset();
		...
		/**
		 *  1.
		 *  获取目标channel的 bufferBuilder
		 *  bufferBuilder 内维护了MemorySegment，即内存片段
		 *  Flink的内存管理依赖 MemorySegment，可实现堆内堆外内存的管理
		...
		/**
		 * 2. 复制serializer的数据到 bufferBuilder 中
		 */
		 ...
		 *  3.
		 *  循环直到数据完全被写入到buffer
		 *  一条数据可能会被写入到多个缓存中，如果缓存不够用，会申请新的缓存
		 *  数据完全写入完毕之时，当前正在操作的缓存是没有写满的
		 */
		while (result.isFullBuffer()) {

			/**
			 * 如果buffer写满了，结束这个 BufferBuilder
			 */
			...
			if (result.isFullRecord()) { // 如果record数据写完了，跳出循环
				pruneTriggered = true;
			...
			/**
			 * 走到这，就代表数据没有全部写入，但是buffer写满了，要重新申请一个 bufferBuilder
			 */
		...
		if (flushAlways) { // 果buffer超时时间为0，需要立即flush目标channel的数据
