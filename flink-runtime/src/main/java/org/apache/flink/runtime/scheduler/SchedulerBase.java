...
public abstract class SchedulerBase implements SchedulerNG {
	...
	public SchedulerBase(
		...
		/**
		 * *******************************************
		 *  非常重要的一步，生成 ExecutionGraph
		 *  在SchedulerNG的构造方法里生成，不容易被关注
		 * *******************************************
		 */
	...
	/**
	 * 生成ExecutionGraph
	 * 非常重要的方法，jobGraph =》 executionGraph，增加并行度
	 */
	...
	public CompletableFuture<String> triggerSavepoint(final String targetDirectory, final boolean cancelJob) {
		mainThreadExecutor.assertRunningInMainThread(); // 确保运行在主线程
		...
		/**
		 * 调用 CheckpointCoordinator 组件
		 */
		return checkpointCoordinator
			/**
			 * 1. 先触发一次savepoint操作（实际上触发的是checkpoint）
			 */
			.triggerSavepoint(System.currentTimeMillis(), targetDirectory)
			/**
			 * 2. 接下来返回checkpoint操作保存的文件路径
			 */
			.thenApply(CompletedCheckpoint::getExternalPointer)
			.handleAsync((path, throwable) -> {
				if (throwable != null) {
					// 3-1. 如果需要取消作业，并且之前步骤抛出了异常，则再次启动checkpoint协调器，抛出异常
					if (cancelJob) {
						startCheckpointScheduler(checkpointCoordinator);
					}
					throw new CompletionException(throwable);
				} else if (cancelJob) {
					// 3-2. 如果需要取消作业，之前步骤没有抛出异常，取消任务执行
	...
	@Override
	public void acknowledgeCheckpoint(final JobID jobID,
