 ...
 *
 * 	代表由一个Task的生成的数据
 * 	和 ExecutionGraph 中的 IntermediateResultPartition 一一对应
 *
 * 	生产者生产的数据首先写入到 ResultPartition 中，一个 Operator 实例对应一个 ResultPartition
 *
 */
public class ResultPartition implements ResultPartitionWriter, BufferPoolOwner {
	...
	public BufferBuilder getBufferBuilder() throws IOException, InterruptedException {
		...
		/**
		 *  从缓存池 bufferPool 以阻塞方式获取BufferBuilder
		 *  此处的 bufferPool 是LocalBufferPool类型
		 */
	...
	public boolean addBufferConsumer(BufferConsumer bufferConsumer, int subpartitionIndex) throws IOException {
		...
		try {
			checkInProduceState();
			// 获取subPartition, 此处 subpartitionIndex 为 targetChannel
		...
		/**
		 *  PipelinedSubpartition: 用于流处理场景下的数据消费, 它内部维护了一个BufferBuilder队列
