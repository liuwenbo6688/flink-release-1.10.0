 ...
 *
 * RpcEndpoint 是对 RPC 框架中提供具体服务的实体的抽象，所有提供远程调用方法的组件都需要继承该抽象类
 *
 * 对于同一个RpcEndpoint的所有RPC调用都会在同一个线程（RpcEndpoint 的“主线程”）中执行，因此无需担心并发执行的线程安全问题
 */
public abstract class RpcEndpoint implements RpcGateway, AutoCloseableAsync {
	...
	/**
	 * RpcService 是 RpcEndpoint 的运行时环境， RpcService 提供了启动 RpcEndpoint, 连接到远端 RpcEndpoint 并返回远端 RpcEndpoint 的代理对象等方法
	 * 实现类是 AkkaRpcService
	 */
	...
	/**
	 * RpcServer 相当于 RpcEndpoint 自身的的代理对象（self gateway)
	 * 实现类 AkkaInvocationHandler | FencedAkkaInvocationHandler
	 */
	...
	public <C extends RpcGateway> C getSelfGateway(Class<C> selfGatewayType) {
		/**
		 * 通过 getSelfGateway 方法就可以获得自身的代理，然后调用该Endpoint 提供的服务
		 */
		if (selfGatewayType.isInstance(rpcServer)) {
			@SuppressWarnings("unchecked")
			C selfGateway = ((C) rpcServer); // rpcServer 是通过动态代理创建的
