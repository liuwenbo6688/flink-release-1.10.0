...
public class TaskManagerServices {
	...
	public static TaskManagerServices fromConfiguration(
		...
		final TaskSlotTable<Task> taskSlotTable = createTaskSlotTable(
			taskManagerServicesConfiguration.getNumberOfSlots(), // taskmanager.numberOfTaskSlots
	...
	private static ShuffleEnvironment<?, ?> createShuffleEnvironment(
		...
		final ShuffleEnvironmentContext shuffleEnvironmentContext = new ShuffleEnvironmentContext(
			...
			/**
			 *  网络（Network）缓存
			 *  用于TaskManager之间（shuffle、广播等）及与外部组件的数据传输，以直接内存形式分配
			 *
			 *  最终体现在 NetworkBufferPool 组件的初始化上
			 */
