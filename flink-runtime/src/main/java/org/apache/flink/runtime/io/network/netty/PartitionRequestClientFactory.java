	...
	NettyPartitionRequestClient createPartitionRequestClient(ConnectionID connectionId) throws IOException, InterruptedException {
		...
		while (client == null) {
			...
			if (entry != null) {
				...
				if (old == null) {
					/**
					 *  ConnectingChannel 作为 Listener 传入
					 *  当连接成功后，会回调 ConnectingChannel.operationComplete() 方法
					 *
					 *
					 */
					...
					/**
					 * 等待连接成功，并且直到 operationComplete() 方法被回调
					 * operationComplete() --> handInChannel() --> 初始化NettyPartitionRequestClient
					 *
					 * waitForChannel() 方法一直阻塞等待 NettyPartitionRequestClient初始化完成
					 */
	...
	private static final class ConnectingChannel implements ChannelFutureListener {
		...
		private void handInChannel(Channel channel) {
			synchronized (connectLock) {
				try {
					/**
					 * 从 channel的 pipeline 中拿到 核心的handler， 就是CreditBasedPartitionRequestClientHandler
					 * 封装到 NettyPartitionRequestClient 中
					 */
		...
		private NettyPartitionRequestClient waitForChannel() throws IOException, InterruptedException {
			...
			/**
			 *  一直阻塞等待  partitionRequestClient 不为null
			 */
			synchronized (connectLock) {
