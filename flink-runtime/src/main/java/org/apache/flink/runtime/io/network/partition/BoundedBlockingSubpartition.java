 ...
 *
 * BoundedBlockingSubpartition 适合批处理场景下的数据消费
 *
 * BoundedBlockingSubpartition 数据是先写入后消费的，可以一次写入，多次消费
 * 它的数据写入到BoundedData中。数据落地的方式随着 BoundedData 实现的不同而不同
 * 数据可以保存在文件系统(FileChannelBoundedData)，内存(MemoryMappedBoundedData)或者同时在文件系统和内存(FileChannelMemoryMappedBoundedData)
 *
