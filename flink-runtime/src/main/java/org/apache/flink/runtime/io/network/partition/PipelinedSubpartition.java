 ...
 *
 * PipelinedSubpartition 用于流处理场景下的数据消费
 *
 * 它内部维护了一个 Buffer 队列
 * 消费者通过调用 createReadView 创建一个PipelinedSubpartitionView来消费数据
 * 创建view的时候需要提供一个 BufferAvailabilityListener 对象，用于作为buffer中有数据可用时候的回调
 * 因此 PipelinedSubpartition 可以做到一旦有数据就及时提醒下游去消费
 *
 */
class PipelinedSubpartition extends ResultSubpartition {
	...
	BufferAndBacklog pollBuffer() {
		synchronized (buffers) {
			...
			if (buffers.isEmpty()) { // 如果buffers队列为空，不需要flush
			...
			while (!buffers.isEmpty()) {
				...
				/**
				 * 转换 bufferConsumer为 Buffer 类型
				 */
				buffer = bufferConsumer.build();

				checkState(bufferConsumer.isFinished() || buffers.size() == 1, // 必须是 true
				...
				if (buffer.readableBytes() > 0) { // 如果可读字节数大于0，跳出循环
					break;
				}
				buffer.recycleBuffer();
				buffer = null;
				if (!bufferConsumer.isFinished()) {
					// 运行到这里只有一种可能，buffer队列中只有一个没有 finished 的buffer，跳出循环
					break;
				}
			}
			...
			updateStatistics(buffer); // 更新统计指标
