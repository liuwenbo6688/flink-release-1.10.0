 ...
 *
 * BufferBuilder 是 MemorySegment的一个封装形式
 *
...
public class BufferBuilder {
	...
	public BufferConsumer createBufferConsumer() {
		...
		/**
		 *  那么 BufferBuilder 和 BufferConsumer 分别是负责什么的呢？
		 *  BufferBuilder是一个辅助类，用于将 java.nio.ByteBuffer 的内容写入到Flink的MemorySegment中 (append方法)
		 *  同样，BufferConsumer也是一个辅助类，用于读取 MemorySegment的数据 (build方法，将数据包装为Buffer类型返回)
		 */
