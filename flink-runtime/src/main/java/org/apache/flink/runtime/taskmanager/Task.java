 ...
 *
 *  Execution 被调度后在分配的 TaskManager 中启动对应的 Task
 *
 *  Task 包裹了具有用户执行逻辑的 operator
 */
public class Task implements Runnable, TaskSlotPayload, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
	...
	/**
	 *  代表Task的输入封装
	 *  每个InputGate消费了一个或多个的ResultPartition
	 *
	 */
	...
	private void doRun() {
		...
		/**
		 *  AbstractInvokable invoke()方法触发计算任务，具体的实现子类
	...
	public void triggerCheckpointBarrier(
		...
		if (executionState == ExecutionState.RUNNING && invokable != null) {
			try {
				 ...
				 *  invokable是具体的Task类型
				 *
				 *  可以是 SourceStreamTask 或 StreamTask
				 *  其中SourceStreamTask的父类是StreamTask，调用的triggerCheckpointAsync也是父类中的方法
				 *
				 *  继续分析StreamTask的 triggerCheckpointAsync() 方法
				 */
