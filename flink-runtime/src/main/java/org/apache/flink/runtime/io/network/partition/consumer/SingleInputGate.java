...
public class SingleInputGate extends InputGate {
	 ...
	 * 一个 InputChannel 接收一个ResultSubpartition的输出
	 */
	private final Map<IntermediateResultPartitionID, InputChannel> inputChannels;

	/** Channels, which notified this input gate about available data. */
	// inputChannelsWithData 中表示该channel有数据需要读取
	...
	public void setup() throws IOException, InterruptedException {
		...
		/**
		 * 为 InputChannel 分配专属 buffer
		 */
		assignExclusiveSegments();

		/**
		 * 调用
		 * NetworkBufferPool#createBufferPool  生成  LocalBufferPool
		 */
	...
	/**
	 *  触发所有的输入通道向 ResultSubpartition 发起请求
	 */
	@VisibleForTesting
		...
		synchronized (requestLock) {
			if (!requestedPartitionsFlag) {
				...
				for (InputChannel inputChannel : inputChannels.values()) {
					/**
					 * 一个 InputChannel 接收一个ResultSubpartition的输出
					 * 触发所有InputChannel发起调用以请求生产端的 ResultSubpartition
					 */
					/**
					 ...
					 * 远程数据交换的通信机制建立在Netty框架的基础之上，
					 * 因此会有一个主交互对象PartitionRequestClient来衔接通信层跟输入通道。
					 *
					 */
	...
	public void assignExclusiveSegments() throws IOException {
		synchronized (requestLock) {
			for (InputChannel inputChannel : inputChannels.values()) {
				if (inputChannel instanceof RemoteInputChannel) { // 只有RemoteInputChannel 才会分配专属buffer
	...
	private Optional<BufferOrEvent> getNextBufferOrEvent(boolean blocking) throws IOException, InterruptedException {
		...
		return Optional.of(
			//
			transformToBufferOrEvent(inputWithData.data.buffer(), inputWithData.moreAvailable, inputWithData.input)
		);
	}
	...
	private Optional<InputWithData<InputChannel, BufferAndAvailability>> waitAndGetNextData(boolean blocking)
		...
		while (true) {
			...
			/**
			 *  获取 InputChannel，根据blocking参数决定是否是阻塞方式
			 */
	...
	private Optional<InputChannel> getChannel(boolean blocking) throws InterruptedException {
		synchronized (inputChannelsWithData) {
			...
			/**
			 *  队列中长度==0，根据blocking判断是等待还是直接返回空
			 */
			...
			// 有可用channel，remove从队列头取出channel
